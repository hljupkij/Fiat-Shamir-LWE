...
// how many bits in higher byte do I need to get a random degree, if degree of reduction polynomial > 256
...
const unsigned int const_BYTES_NUMBER = ceil(ceil(log2(to_double(count_elements_up_to_L1(PUBLIC_PARAMETER_N,PUBLIC_PARAMETER_K))))/8);
...
ZZ_pE RandomOracle_Dc(
	...
	strncpy((char *)&message_with_lwe[message_length],(const char*)char_arr, length_char_arr);

	SHA512(message_with_lwe, message_length+length_char_arr, md_value);
	md_value[const_BYTES_NUMBER-1] &= 1;		// remove first 7 bits to get a number in range 0...(2^161)-1

	ZZ number = ZZFromBytes(md_value, const_BYTES_NUMBER);	// 8*21=168-Bit with
...
int verify_signatur(
	...
	if(!compare_ZZ_pE(SignaturePart_e,&temp_e,1)){
		return 0;
	}
	return -1;
}
...
int sign_message(
	...
	while(!element_ok && count < 30){
		...
		for (int index = 0; index < PublicParam.m; ++index) {
			...
			if(Linf_norm_ZZ_pE(SignaturePart_z[index]) > max_length){
				element_ok *= false;
				break;
			}
		}
